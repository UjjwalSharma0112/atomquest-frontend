...
export default function AnalyticsPage() {
  ...
  const { data: qoq, isLoading: qoqLoading } = useAnalyticsQoQ(
    effectiveCycleId || undefined,
    selectedEmployeeId || undefined,
  );
  const { data: distribution, isLoading: distLoading } =
    useAnalyticsDistribution(effectiveCycleId || undefined);
  const { data: heatmap, isLoading: heatLoading } = useAnalyticsHeatmap(
    effectiveCycleId || undefined,
  );
  const { data: managers, isLoading: mgrLoading } = useAnalyticsManagers(
    isAdmin ? effectiveCycleId || undefined : undefined,
  );
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-text-muted text-sm mt-1">
            Organisation performance insights
          </p>
        </div>
        <div className="w-56">
          <Select
        ...
        {overviewLoading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <StatIconCard
              ...
              label="Approved"
              value={overview.goals.approved}
              color="text-success"
            />
            <StatIconCard
              icon={<Clock size={16} />}
              label="Pending"
              value={overview.goals.pending}
              color="text-warning"
            />
            <StatIconCard
              icon={<Activity size={16} />}
              label="Check-ins"
              value={overview.checkIns.total}
              color="text-primary"
            />
            <StatIconCard
              icon={<TrendingUp size={16} />}
              label="Completion"
              value={overview.checkIns.completionRate}
              color="text-primary"
            />
          </div>
        ) : (
          <Card className="p-8">
            <EmptyState
              ...
              description="Select a cycle to view analytics"
            />
          </Card>
        )}
      </section>
        ...
        {qoqLoading ? (
              ...
              >
                ...
                <Tooltip
                  ...
                  formatter={(value: number, name: string) => [
                    ...
                    name === "averageScore" ? "Avg Score" : "Goals Checked In",
                  ]}
                />
                <Line
                  type="monotone"
          ...
          <Card className="p-8">
            <EmptyState
              ...
              title="No QoQ data"
              description="Select an employee to view quarter trends"
            />
          </Card>
        )}
      </section>
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                      <Pie
                        ...
                        stroke="none"
                      >
                        {distribution.byUom.map((_: unknown, i: number) => (
                          <Cell
                            key={i}
                ...
                <ResponsiveContainer width="100%" height={220}>
                  ...
                  >
                    ...
                    <Bar dataKey="count" radius={[4, 4, 0, 0]}>
                      {distribution.byStatus.map((d: { status: string }) => (
                        <Cell
                          key={d.status}
        ...
        {heatLoading ? (
                  ...
                  <div className="px-4 py-3 bg-bg-elevated border-b border-bg-border flex items-center justify-between">
                    ...
                    <span className="text-xs text-text-muted">
                      ...
                      <span className="font-mono font-bold text-primary">
                        {mgr.teamAverageCompletionRate.toFixed(1)}%
                      </span>
                    </span>
                  </div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-bg-border">
                          ...
                          <th className="text-center px-3 py-2.5 text-xs font-semibold text-text-muted uppercase tracking-wide w-20">
                            Q1
                          </th>
                          <th className="text-center px-3 py-2.5 text-xs font-semibold text-text-muted uppercase tracking-wide w-20">
                            Q2
                      ...
                      <tbody className="divide-y divide-bg-border">
                        {mgr.employees.map((emp) => {
                          ...
                          emp.quarterStats.forEach((qs) => {
                            qMap[qs.quarter] = qs.completionRate;
                          });
                          return (
                            <tr
                              key={emp.employeeId}
                            ...
                            >
                              ...
                              {["Q1", "Q2", "Q3", "Q4"].map((q) => {
                                ...
                                return (
                                  <td key={q} className="px-3 py-3 text-center">
                                    <span
                                      className={`inline-block w-14 text-center py-1.5 rounded-md text-xs font-mono font-bold ${heatColor(val)}`}
                                    >
                                      {val}%
                                    </span>
                                  </td>
                              ...
                              <td className="px-3 py-3 text-center">
                                <span
                                  className={`inline-block w-14 text-center py-1.5 rounded-md text-xs font-mono font-bold ${heatColor(emp.overallCompletionRate)}`}
                                >
          ...
          <Card className="p-8">
            <EmptyState
              ...
              description="No team completion data available"
            />
          </Card>
        )}
      </section>
          ...
          {mgrLoading ? (
                  ...
                  <tbody className="divide-y divide-bg-border">
                    {managers.map(
                      (m: {
                        ...
                        totalApprovedGoals: number;
                        approvalRate: string;
                        checkInsWithManagerComment: number;
                        ...
                        >
                          ...
                          <td className="px-3 py-3 text-center font-mono text-text-primary">
                            {m.teamSize}
                          </td>
                          <td className="px-3 py-3 text-center font-mono text-text-primary">
                            {m.totalApprovedGoals}
                          </td>
                          <td className="px-3 py-3 text-center">
                            <span
                              className={`font-mono font-bold text-xs ${rateColor(m.approvalRate)}`}
                            >
                              {m.approvalRate}
                            </span>
                          </td>
                          <td className="px-3 py-3 text-center font-mono text-text-primary">
                            {m.checkInsWithManagerComment}
                          </td>
                          <td className="px-3 py-3 text-center">
                            <span
                              className={`font-mono font-bold text-xs ${rateColor(m.checkInCommentRate)}`}
                            >
                              {m.checkInCommentRate}
                            </span>
                          </td>
                        </tr>
                      ),
            ...
            <Card className="p-8">
              <EmptyState
                ...
                description="Manager effectiveness data is not available"
              />
            </Card>
          )}
        </section>
